...
const TRACE_KEY = "boralog-auth-018-standard-trace";
...
export function AuthForm({ mode, initialFeedback }: { mode: Mode; initialFeedback?: string }) {
  ...
  function handleStandardLoginSubmit(event: FormEvent<HTMLFormElement>) {
    if (signup) return;
    trace("standard_submit_event", event.nativeEvent.isTrusted);
  ...
  return <form
    ...
    onSubmit={signup ? undefined : handleStandardLoginSubmit}
    aria-busy={pending}
  >
    ...
    {!signup && <pre data-runtime-trace="boralog-auth-018-standard" aria-label="Trace technique temporaire">{runtimeTrace.length ? runtimeTrace.join("\\n") : "0 trace_ready"}</pre>}
